...
public class TempMuteCMD extends Command {
    public TempMuteCMD(){
        super("tempmute", null, new String[]{"minecraft:tempmute","bukkit:tempmute","essentials:tempmute"});
    }
            ...
            if (u.hasPermission(Rank.MOD)) {
                if(args.length >= 3){
                    ...
                    if(uuid != null){
                            ...
                            if(BungeeChest.getInstance().isValidInteger(timeString) && Integer.parseInt(timeString) > 0){
                                if(!BanUtilities.isMuted(uuid)){
                                    ...
                                    if(reason == null){
                                        ...
                                        for(ProxiedPlayer all : PlayerUtilities.getOnlineStaff(Rank.MOD)){
                                            BungeeUser a = BungeeUser.getUser(all);
                                            all.sendMessage(TextComponent.fromLegacyText(ChatColor.DARK_RED + "[BANS] " + a.getTranslatedMessage("%p muted %u with reason %r for %t.").replace("%p",u.getRank().getColor() + p.getName() + ChatColor.YELLOW).replace("%u",rank.getColor() + playerName + ChatColor.YELLOW).replace("%r",ChatColor.WHITE + reason + ChatColor.YELLOW).replace("%t",ChatColor.WHITE.toString() + time + timeUnit + ChatColor.YELLOW)));
                                        }
                                    }
                                } else {
                        ...
                        p.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.RED + u.getTranslatedMessage("cmd.unknownUUID")));
                    }
                } else {
                    p.sendMessage(TextComponent.fromLegacyText(ChatColor.DARK_RED + "[BANS] " + ChatColor.RED + "/tempmute <Player> <Time> <TimeUnit> [Reason]"));
                }
            } else {
                    ...
                    if(true){
                        ...
                        if(BungeeChest.getInstance().isValidInteger(timeString) && Integer.parseInt(timeString) > 0){
                            if(!BanUtilities.isMuted(uuid)){
                                ...
                                try {
                                    PreparedStatement second = MySQLManager.getInstance().getConnection().prepareStatement("INSERT INTO `sn_mutes` (`mutedPlayer`, `reason`, `nameWhenMuted`, `server`, `time`, `expiry`) VALUES(?,?,?,?,?,?)");
                                    second.setString(1, uuid.toString());
                                    second.setString(2, reason);
                                    second.setString(3, playerName);
                                    second.setString(4, server);
                                    second.setTimestamp(5, new Timestamp(System.currentTimeMillis()));
                                    second.setTimestamp(6, endDate);
                                    second.execute();
                                    second.close();
                                } catch(Exception e){
                                    e.printStackTrace();
                                ...
                                if(reason == null){
                                    ...
                                    for(ProxiedPlayer all : PlayerUtilities.getOnlineStaff(Rank.MOD)){
                                        BungeeUser a = BungeeUser.getUser(all);
                                        all.sendMessage(TextComponent.fromLegacyText(ChatColor.DARK_RED + "[BANS] " + a.getTranslatedMessage("%p muted %u with reason %r for %t.").replace("%p",ChatColor.DARK_PURPLE + "BungeeConsole" + ChatColor.YELLOW).replace("%u",rank.getColor() + playerName + ChatColor.YELLOW).replace("%r",ChatColor.WHITE + reason + ChatColor.YELLOW).replace("%t",ChatColor.WHITE.toString() + time + timeUnit + ChatColor.YELLOW)));
                                    }
                                }
                            } else {
