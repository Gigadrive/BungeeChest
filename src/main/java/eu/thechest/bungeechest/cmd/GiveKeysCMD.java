 ...
 */
public class GiveKeysCMD extends Command {
    public GiveKeysCMD(){
        super("givekeys");
    }
    ...
    public void execute(CommandSender sender, String[] args) {
        if(sender instanceof ProxiedPlayer){
            ...
            if(u.hasPermission(Rank.ADMIN)){
                if(args.length == 2){
                    ...
                    if(BungeeChest.getInstance().isValidInteger(args[1])){
                        ...
                        if(uuid != null){
                            try {
                                PreparedStatement ps = MySQLManager.getInstance().getConnection().prepareStatement("UPDATE `users` SET `keys`=`keys`+? WHERE `uuid`=?");
                                ps.setInt(1,amount);
                                ...
                                if(updateAmount > 0){
                                    p.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.GREEN + u.getTranslatedMessage("The keys have been credited.")));
                                } else {
                                    p.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.YELLOW + u.getTranslatedMessage("The keys may not have been credit as that user was not found in the database.")));
                                }
                            } catch(Exception e){
                                e.printStackTrace();
                            }
                    ...
                    p.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.RED + "/givekeys <" + u.getTranslatedMessage("Player") + "> <" + u.getTranslatedMessage("Amount") + ">"));
                }
            } else {
            ...
            if(args.length == 2){
                ...
                if(BungeeChest.getInstance().isValidInteger(args[1])){
                    ...
                    if(uuid != null){
                        try {
                            ...
                            if(updateAmount > 0){
                                sender.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.GREEN + "The keys have been credited."));
                            } else {
                                sender.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.YELLOW + "The keys may not have been credit as that user was not found in the database."));
                            }
                        } catch(Exception e){
                            e.printStackTrace();
                        }
                ...
                sender.sendMessage(TextComponent.fromLegacyText(BungeeChest.PREFIX + ChatColor.RED + "/givekeys <Player> <Amount>"));
            }
        }
    }
}
